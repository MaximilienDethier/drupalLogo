...
class FieldMacro extends MacroBase {
  ...
  public function prepareView(array $context) {
    ...
    if (empty($this->params['id'])) {
      ...
      list($this->entityID, $vid, $this->bundle) = entity_extract_ids($context['entity_type'], $context['entity']);
    }
    elseif (!empty($this->params['type']) && !empty($this->params['id'])) {
      ...
      list($this->entityID, $vid, $this->bundle) = entity_extract_ids($this->entityType, $this->entity);
    }
    // @todo Proper error handling.
  ...
  public function view(array $context) {
    ...
    $instance = field_info_instance($this->entityType, $field_name, $this->bundle);
